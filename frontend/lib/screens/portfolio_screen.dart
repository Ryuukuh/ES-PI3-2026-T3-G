...
class _PortfolioScreenState extends State<PortfolioScreen> {
  ...
  // 0=Diário, 1=Semanal, 2=Mensal, 3=6 meses, 4=YTD
  ...
  /// Gera pontos simulados de variação de valorização baseados no total investido.
  /// Usa seed por período para os valores serem consistentes entre renders.
  List<FlSpot> _gerarPontosGrafico(double totalInvestido, int periodo) {
    if (totalInvestido <= 0) return [];

    final int numPontos;
    switch (periodo) {
      case 0: numPontos = 24; break; // diário = 24 horas
      case 1: numPontos = 7;  break; // semanal = 7 dias
      case 2: numPontos = 30; break; // mensal = 30 dias
      case 3: numPontos = 26; break; // 6 meses = 26 semanas
      case 4:                        // YTD = semanas desde jan
    ...
    for (int i = 0; i < numPontos; i++) {
      // Variação entre -3% e +5% (leve tendência de alta)
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (q > 0) Navigator.of(context).pop(true);
            },
  ...
  Future<void> _realizarDeposito(double valor) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _userData['saldoFicticio'] = novoSaldo;
        });
        await SessionService.saveUser(_userData);
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  /// Retorna o label do eixo X conforme o período e o índice do ponto.
  String _xLabel(int i) {
    switch (_selectedPeriod) {
      case 0: // Diário — 24 horas
        return i % 4 == 0 ? '${i}h' : '';
      case 1: // Semanal — 7 dias
        const dias = ['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'];
        return i < dias.length ? dias[i] : '';
      case 2: // Mensal — 30 dias
        if (i == 0) return '1';
        if ((i + 1) % 5 == 0) return '${i + 1}';
        return '';
      case 3: // 6 meses — 26 semanas
        if (i % 4 != 0) return '';
      ...
      case 4: // Anual — semanas desde jan
        if (i % 4 != 0) return '';
              ...
              lineTouchData: LineTouchData(
                ...
                getTouchedSpotIndicator: (barData, spotIndexes) => spotIndexes.map((_) {
                  return TouchedSpotIndicatorData(
                    FlLine(color: lineColor.withAlpha(120), strokeWidth: 1.5, dashArray: [4, 4]),
                    FlDotData(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Distribuição por startup (pizza)
